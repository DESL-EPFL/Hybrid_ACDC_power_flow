function [H16, H26, H36, H46, H56, H61, H62, H63, H64, H65, H66] = d2St_dxqtma2(branch, V, mu, vcart)
%D2ST_DXQTMA2  Computes 2nd derivatives of complex brch power flow "to" w.r.t. qtmaVa, qtmaVm, qtmaBeqz, qtmaBeqv, qtmaSh, Vaqtma, Vmqtma, Beqzqtma, Beqvqtma, Shqtma, qtmaqtma.
%
...
%
%   [HqtmaVa, HqtmaVm, HqtmaBz, HqtmaBv, HqtmaSh, HVaqtma, HVmqtma, HBzqtma, HBvqtma, HShqtma, Hqtmaqtma] = D2ST_DXQTMA2(BRANCH, V, MU)
%   [HqtmaVa, HqtmaVm, HqtmaBz, HqtmaBv, HqtmaSh, HVaqtma, HVmqtma, HBzqtma, HBvqtma, HShqtma, Hqtmaqtma] = D2ST_DXQTMA2(BRANCH, V, MU, 0)
%
...
%
%   [HqtmaVa, HqtmaVm, HqtmaBz, HqtmaBv, HqtmaSh, HVaqtma, HVmqtma, HBzqtma, HBvqtma, HShqtma, Hqtmaqtma] = D2ST_DXQTMA2(BRANCH, V, MU, 1)
%
%   Not Coded Yet
%
%   Examples:
%       il = find(branch(:, RATE_A) ~= 0 & branch(:, RATE_A) < 1e10);
%       [HqtmaVa, HqtmaVm, HqtmaBz, HqtmaBv, HqtmaSh, HVaqtma, HVmqtma, HBzqtma, HBvqtma, HShqtma, Hqtmaqtma] = d2St_dxqtma2(branch(il,:), V, mu, vcart);
%
%       Here the output matrices correspond to:
%           HqtmaVa   = d/dqtma (dSt_dVa.'   * mu)
%           HqtmaVm   = d/dqtma (dSt_dVm.'   * mu)
%           HqtmaBz   = d/dqtma (dSt_dBeqz.' * mu)
%           HqtmaBv   = d/dqtma (dSt_dBeqv.' * mu)
%           HqtmaSh   = d/dqtma (dSt_dSh.'   * mu)
%           HVaqtma   = d/dVa   (dSt_dqtma.' * mu)
%           HVmqtma   = d/dVm   (dSt_dqtma.' * mu)
%           HBzqtma   = d/dBeqz (dSt_dqtma.' * mu)
%           HBvqtma   = d/dBeqv (dSt_dqtma.' * mu)
%           HShqtma   = d/dSh   (dSt_dqtma.' * mu)
%           Hqtmaqtma = d/dqtma (dSt_dqtma.' * mu)
%
...
if vcart
    error('d2St_dxqtma2: Derivatives of Flow Limit equations w.r.t ma in cartasian have not been coded yet')    
    ...
    d2St_dqtmash   = sparse( zeros(nPfsh,nQtma) );    
    ...
    for k=1:nQtma
        for kk=1:nb %dQtmaVx
            ...
            %2nd Derivatives of St w.r.t. ShVx
            d2St_dqtmaVa(kk, k) = ((diag(Ct*V)*conj(dYt_dma*dVa(:,kk))   +   (Ct*dVa(:,kk)).*conj(dYt_dma*V)).')*mu; %AAB- Final dSt_dmaVa has a size of [nl, nQtma] 
            d2St_dqtmaVm(kk, k) = ((diag(Ct*V)*conj(dYt_dma*dVm(:,kk))   +   (Ct*dVm(:,kk)).*conj(dYt_dma*V)).')*mu; %AAB- Final dSt_dmaVm has a size of [nl, nQtma]  
        end
        for kk=1:nBeqz
            %% Second Derivatives %The shBeqz derivative is zero because Yft, Ytf and Ytt do not share Theta_shift and Beqz for any case.
            ...
            d2St_dqtmaBeqz(kk,k) = (diag(Ct*V)*conj(d2Yt_dmaBeqz*V)).'*mu;   %AAB- must be zero  
            
        end
        for kk=1:nBeqv
            %% Second Derivatives %The maBeqv derivative is zero because Yft, Ytf and Ytt do not share ma and Beqv for any case.
            ...
            d2St_dqtmaBeqv(kk,k) = (diag(Ct*V)*conj(d2Yt_dmaBeqv*V)).'*mu;   %AAB- must be zero 
        end
        for kk=1:nPfsh
            ...
            d2St_dqtmash(kk,k) = (diag(Ct*V)*conj(d2Yt_dmash*V)).'*mu;                  
        end
        for kk=1:nQtma
            ...
            d2St_dqtma2(kk,k) = (diag(Ct*V)*conj(d2Yt_dma2*V)).'*mu;         %AAB- size of [nQtma, nQtma]   
...
H16 = sparse(d2St_dqtmaVa);
H26 = sparse(d2St_dqtmaVm);
H36 = sparse(d2St_dqtmaBeqz);
H46 = sparse(d2St_dqtmaBeqv);
H56 = sparse(d2St_dqtmash);
H61 = H16.';
H62 = H26.';
H63 = H36.';
H64 = H46.';
H65 = H56.';
H66 = sparse(d2St_dqtma2);
