...
%D2SF_DXSHDP2PERT  Computes 2nd derivatives of complex brch power flow "from" w.r.t. ShdpVa, ShdpVm, ShdpBeqz, ShdpBeqv, ShdpSh, Shdpqtma, Shdpvtma, VaShdp, VmShdp, BeqzShdp, BeqvShdp, ShShdp, qtmaShdp, vtmaShdp, ShdpShdp (Finite Differences Method).
%
...
%   
%   [HfShdpVa, HfShdpVm, HfShdpBz, HfShdpBv, HfShdpSh, HfShdpqtma, HfShdpvtma, HfVaShdp, HfVmShdp, HfBzShdp, HfBvShdp, HfShShdp, HfqtmaShdp, HfvtmaShdp, HfShdpShdp, ...
%   [HtShdpVa, HtShdpVm, HtShdpBz, HtShdpBv, HtShdpSh, HtShdpqtma, HtShdpvtma, HtVaShdp, HtVmShdp, HtBzShdp, HtBvShdp, HtShShdp, HtqtmaShdp, HtvtmaShdp, HtShdpShdp] = D2SF_DXSHDP2(BASEMVA, BUS, BRANCH, V, LAM, PERT, 0)
%
%   Returns 30 matrices containing the partial derivatives w.r.t. Va, Vm,
%   Beq, Sh and ma of the product of a vector LAM with the 1st partial 
%   derivatives of the complex branch power flows.
%
%   [HfShdpVa, HfShdpVm, HfShdpBz, HfShdpBv, HfShdpSh, HfShdpqtma, HfShdpvtma, HfVaShdp, HfVmShdp, HfBzShdp, HfBvShdp, HfShShdp, HfqtmaShdp, HfvtmaShdp, HfShdpShdp, ...
%   [HtShdpVa, HtShdpVm, HtShdpBz, HtShdpBv, HtShdpSh, HtShdpqtma, HtShdpvtma, HtVaShdp, HtVmShdp, HtBzShdp, HtBvShdp, HtShShdp, HtqtmaShdp, HtvtmaShdp, HtShdpShdp] = D2SF_DXSHDP2(BASEMVA, BUS, BRANCH, V, LAM, PERT, 1)
%
%   Not Coded Yet
%
%   Examples:
...
if vcart
    error('d2Sf_dxshdp2Pert: Derivatives of Flow Limit equations w.r.t Theta_dp using Finite Differences Method in cartasian have not been coded yet')    
    ...
    d2Sf_dPfdpVa   = sparse( zeros(nb,   nPfdp) ); 
    d2Sf_dPfdpVm   = sparse( zeros(nb,   nPfdp) );
    d2Sf_dPfdpBeqz = sparse( zeros(nBeqz,nPfdp) );
    d2Sf_dPfdpBeqv = sparse( zeros(nBeqv,nPfdp) );
    d2Sf_dPfdpPfsh = sparse( zeros(nPfsh,nPfdp) );   
    d2Sf_dPfdpQtma = sparse( zeros(nQtma,nPfdp) );  
    d2Sf_dPfdpVtma = sparse( zeros(nVtma,nPfdp) );
    
    d2Sf_dVaPfdp   = sparse( zeros(nPfdp,nb   ) ); 
    d2Sf_dVmPfdp   = sparse( zeros(nPfdp,nb   ) );
    d2Sf_dBeqzPfdp = sparse( zeros(nPfdp,nBeqz) );
    d2Sf_dBeqvPfdp = sparse( zeros(nPfdp,nBeqv) );
    d2Sf_dPfshPfdp = sparse( zeros(nPfdp,nPfsh) );   
    d2Sf_dQtmaPfdp = sparse( zeros(nPfdp,nQtma) );     
    d2Sf_dVtmaPfdp = sparse( zeros(nPfdp,nVtma) );
    
    d2Sf_dPfdp2    = sparse( zeros(nPfdp,nPfdp) );

    d2St_dPfdpVa   = sparse( zeros(nb,   nPfdp) ); 
    d2St_dPfdpVm   = sparse( zeros(nb,   nPfdp) );
    d2St_dPfdpBeqz = sparse( zeros(nBeqz,nPfdp) );
    d2St_dPfdpBeqv = sparse( zeros(nBeqv,nPfdp) );
    d2St_dPfdpPfsh = sparse( zeros(nPfsh,nPfdp) );   
    d2St_dPfdpQtma = sparse( zeros(nQtma,nPfdp) );  
    d2St_dPfdpVtma = sparse( zeros(nVtma,nPfdp) );
    
    d2St_dVaPfdp   = sparse( zeros(nPfdp,nb   ) ); 
    d2St_dVmPfdp   = sparse( zeros(nPfdp,nb   ) );
    d2St_dBeqzPfdp = sparse( zeros(nPfdp,nBeqz) );
    d2St_dBeqvPfdp = sparse( zeros(nPfdp,nBeqv) );
    d2St_dPfshPfdp = sparse( zeros(nPfdp,nPfsh) );   
    d2St_dQtmaPfdp = sparse( zeros(nPfdp,nQtma) );     
    d2St_dVtmaPfdp = sparse( zeros(nPfdp,nVtma) );
    
    d2St_dPfdp2    = sparse( zeros(nPfdp,nPfdp) );
    ...
    for k = 1:nb
        ...
        d2Sf_dVaPfdp(:, k) = (dSf_dPfdp_PertVa - dSf_dPfdp).' * lam / pert; %PfdpVa from, size of [nPfdp, nb] 
        d2St_dVaPfdp(:, k) = (dSt_dPfdp_PertVa - dSt_dPfdp).' * lam / pert; %PfdpVa  to , size of [nPfdp, nb] 
    end
    %PfdpVm num_G82
    for k = 1:nb
        V2p = V;
        ...
        d2Sf_dVmPfdp(:, k) = (dSf_dPfdp_PertVm - dSf_dPfdp).' * lam / pert; %PfdpVm from, size of [nPfdp, nb] 
        d2St_dVmPfdp(:, k) = (dSt_dPfdp_PertVm - dSt_dPfdp).' * lam / pert; %PfdpVm  to , size of [nPfdp, nb] 
    end
    %PfdpBeqz num_G83
    for k=1:nBeqz 
        ...
        %2nd Derivatives of Sbr w.r.t. PfdpBeqz
        d2Sf_dBeqzPfdp(:, k) = (dSf_dPfdp_PertBeqz - dSf_dPfdp).' * lam / pert;  %PfdpBeqz from, size of [nPfdp, nBeqz]
        d2St_dBeqzPfdp(:, k) = (dSt_dPfdp_PertBeqz - dSt_dPfdp).' * lam / pert;  %PfdpBeqz  to , size of [nPfdp, nBeqz]
    end
    %PfdpBeqv num_G84
    for k=1:nBeqv 
        ...
        %2nd Derivatives of Sbr w.r.t. PfdpBeqv
        d2Sf_dBeqvPfdp(:, k) = (dSf_dPfdp_PertBeqv - dSf_dPfdp).' * lam / pert;  %PfdpBeqv from, size of [nPfdp, nBeqv]
        d2St_dBeqvPfdp(:, k) = (dSt_dPfdp_PertBeqv - dSt_dPfdp).' * lam / pert;  %PfdpBeqv  to , size of [nPfdp, nBeqv]
    end
    %PfdpPfsh num_G85
    for k=1:nPfsh 
        ...
        %2nd Derivatives of Sbr w.r.t. PfdpPfsh
        d2Sf_dPfshPfdp(:, k) = (dSf_dPfdp_PertPfsh - dSf_dPfdp).' * lam / pert;  %PfdpPfsh from, size of [nPfdp, nPfsh] 
        d2St_dPfshPfdp(:, k) = (dSt_dPfdp_PertPfsh - dSt_dPfdp).' * lam / pert;  %PfdpPfsh  to , size of [nPfdp, nPfsh] 
    end    
    %PfdpQtma num_G86
    for k=1:nQtma 
        ...
        %2nd Derivatives of Sbr w.r.t. PfdpQtma
        d2Sf_dQtmaPfdp(:, k) = (dSf_dPfdp_PertQtma - dSf_dPfdp).' * lam / pert;  %PfdpQtma from, size of [nPfdp, nQtma] 
        d2St_dQtmaPfdp(:, k) = (dSt_dPfdp_PertQtma - dSt_dPfdp).' * lam / pert;  %PfdpQtma  to , size of [nPfdp, nQtma] 
    end 
    %PfdpVtma num_G87
    for k=1:nVtma 
        ...
        d2Sf_dVtmaPfdp(:, k) = (dSf_dPfdp_PertVtma - dSf_dPfdp).' * lam / pert;  %PfdpVtma from, size of [nPfdp, nVtma] 
        d2St_dVtmaPfdp(:, k) = (dSt_dPfdp_PertVtma - dSt_dPfdp).' * lam / pert;  %PfdpVtma  to , size of [nPfdp, nVtma] 
    end 
    ...
    for k=1:nPfdp 
        ...
        d2Sf_dPfdpVa(:, k) = (dSf_dV1_PertPfdp - dSf_dV1).' * lam / pert;  %VaVtma from, size of [nb, nPfdp] 
        d2Sf_dPfdpVm(:, k) = (dSf_dV2_PertPfdp - dSf_dV2).' * lam / pert;  %VmVtma  to , size of [nb, nPfdp] 
        d2St_dPfdpVa(:, k) = (dSt_dV1_PertPfdp - dSt_dV1).' * lam / pert;  %VaVtma from, size of [nb, nPfdp] 
        d2St_dPfdpVm(:, k) = (dSt_dV2_PertPfdp - dSt_dV2).' * lam / pert;  %VmVtma  to , size of [nb, nPfdp] 
    end
    %BeqzPfdp num_G38
    for k=1:nPfdp 
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
        ...
        d2Sf_dPfdpBeqz(:, k) = (dSf_dBeqz_PertPfdp - dSf_dBeqz).' * lam / pert;  %BeqzPfdp from, size of [nBeqz, nPfdp] 
        d2St_dPfdpBeqz(:, k) = (dSt_dBeqz_PertPfdp - dSt_dBeqz).' * lam / pert;  %BeqzPfdp  to , size of [nBeqz, nPfdp] 
    end
    %BeqvPfdp num_G48
    for k=1:nPfdp 
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
        ...
        d2Sf_dPfdpBeqv(:, k) = (dSf_dBeqv_PertPfdp - dSf_dBeqv).' * lam / pert;  %BeqvPfdp from, size of [nBeqv, nPfdp] 
        d2St_dPfdpBeqv(:, k) = (dSt_dBeqv_PertPfdp - dSt_dBeqv).' * lam / pert;  %BeqvPfdp  to , size of [nBeqv, nPfdp]
    end
    %PfshPfdp num_G57
    for k=1:nPfdp
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
        ...
        d2Sf_dPfdpPfsh(:, k) = (dSf_dPfsh_PertPfdp - dSf_dPfsh).' * lam / pert;  %PfshPfdp from, size of [nPfsh , nPfdp] 
        d2St_dPfdpPfsh(:, k) = (dSt_dPfsh_PertPfdp - dSt_dPfsh).' * lam / pert;  %PfshPfdp  to , size of [nPfsh , nPfdp] 
    end
    %QtmaPfdp num_G68
    for k=1:nPfdp
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
        ...
        d2Sf_dPfdpQtma(:, k) = (dSf_dQtma_PertPfdp - dSf_dQtma).' * lam / pert;  %QtmaPfdp from, size of [nQtma , nPfdp] 
        d2St_dPfdpQtma(:, k) = (dSt_dQtma_PertPfdp - dSt_dQtma).' * lam / pert;  %QtmaPfdp  to , size of [nQtma , nPfdp] 
    end
    %VtmaPfdp num_G78
    for k=1:nPfdp
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
        ...
        d2Sf_dPfdpVtma(:, k) = (dSf_dVtma_PertPfdp - dSf_dVtma).' * lam / pert;  %VtmaPfdp from, size of [nVtma , nPfdp] 
        d2St_dPfdpVtma(:, k) = (dSt_dVtma_PertPfdp - dSt_dVtma).' * lam / pert;  %VtmaPfdp  to , size of [nVtma , nPfdp] 
    end
    %Pfdp2 num_G88
    for k=1:nPfdp
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
        ...
        d2Sf_dPfdp2(:, k) = (dSf_dPfdp_PertPfdp - dSf_dPfdp).' * lam / pert;  %Pfdp2 from, size of [nPfdp , nPfdp] 
        d2St_dPfdp2(:, k) = (dSt_dPfdp_PertPfdp - dSt_dPfdp).' * lam / pert;  %Pfdp2  to , size of [nPfdp , nPfdp]  
    end
end

%Assigning the partial derivatives with their respective outputs. 
num_Hf18 = sparse(d2Sf_dPfdpVa);
num_Hf28 = sparse(d2Sf_dPfdpVm);
num_Hf38 = sparse(d2Sf_dPfdpBeqz);
num_Hf48 = sparse(d2Sf_dPfdpBeqv);
num_Hf58 = sparse(d2Sf_dPfdpPfsh);
num_Hf68 = sparse(d2Sf_dPfdpQtma);
num_Hf78 = sparse(d2Sf_dPfdpVtma);

num_Hf81 = sparse(d2Sf_dVaPfdp);
num_Hf82 = sparse(d2Sf_dVmPfdp);
num_Hf83 = sparse(d2Sf_dBeqzPfdp);
num_Hf84 = sparse(d2Sf_dBeqvPfdp);
num_Hf85 = sparse(d2Sf_dPfshPfdp);
num_Hf86 = sparse(d2Sf_dQtmaPfdp);
num_Hf87 = sparse(d2Sf_dVtmaPfdp);

num_Hf88 = sparse(d2Sf_dPfdp2);

num_Ht18 = sparse(d2St_dPfdpVa);
num_Ht28 = sparse(d2St_dPfdpVm);
num_Ht38 = sparse(d2St_dPfdpBeqz);
num_Ht48 = sparse(d2St_dPfdpBeqv);
num_Ht58 = sparse(d2St_dPfdpPfsh);
num_Ht68 = sparse(d2St_dPfdpQtma);
num_Ht78 = sparse(d2St_dPfdpVtma);

num_Ht81 = sparse(d2St_dVaPfdp);
num_Ht82 = sparse(d2St_dVmPfdp);
num_Ht83 = sparse(d2St_dBeqzPfdp);
num_Ht84 = sparse(d2St_dBeqvPfdp);
num_Ht85 = sparse(d2St_dPfshPfdp);
num_Ht86 = sparse(d2St_dQtmaPfdp);
num_Ht87 = sparse(d2St_dVtmaPfdp);

num_Ht88 = sparse(d2St_dPfdp2);
