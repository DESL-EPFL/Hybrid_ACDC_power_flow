function [H13, H23, H31, H32, H33] = d2Sf_dxBeqz2(branch, V, mu, vcart)
%D2SF_DXBEQZ2  Computes 2nd derivatives of complex brch power flow w.r.t. BeqzVa, BeqzVm, VaBeqz, VmBeqz, BeqzBeqz.
%
...
%
%   [HBzVa, HBzVm, HVaBz, HVmBz, HBzBz] = D2SF_DXBEQZ2(BRANCH, V, MU)
%   [HBzVa, HBzVm, HVaBz, HVmBz, HBzBz] = D2SF_DXBEQZ2(BRANCH, V, MU, 0)
%
...
%
%   [HBzVa, HBzVm, HVaBz, HVmBz, HBzBz] = D2SF_DXBEQZ2(BRANCH, V, MU, 1)
%
%   Not Coded Yet
%
%   Examples:
%       il = find(branch(:, RATE_A) ~= 0 & branch(:, RATE_A) < 1e10);
%       [HBzVa, HBzVm, HVaBz, HVmBz, HBzBz] = d2Sf_dxBeqz2(branch(il,:), V, mu, vcart);
%
%       Here the output matrices correspond to:
%           HBzVa = d/dBeqz (dSf_dVa.'   * mu)
%           HBzVm = d/dBeqz (dSf_dVm.'   * mu)
%           HVaBz = d/dVa   (dSf_dBeqz.' * mu)
%           HVmBz = d/dVm   (dSf_dBeqz.' * mu)
%           HBzBz = d/dBeqz (dSf_dBeqz.' * mu)
%
...
if vcart
    ...
    for k=1:nBeqz 
        for kk=1:nb %dBeqzVx
            ...
            d2Sf_dBeqzVa(kk, k) = ((diag(Cf*V)*conj(dYf_dBeq*dVa(:,kk))   +  (Cf*dVa(:,kk)).*conj(dYf_dBeq*V)).')*mu;  %AAB- Final dSf_dBeqVa has a size of [nl, nBeqx]
            d2Sf_dBeqzVm(kk, k) = ((diag(Cf*V)*conj(dYf_dBeq*dVm(:,kk))   +  (Cf*dVm(:,kk)).*conj(dYf_dBeq*V)).')*mu; %AAB- Final dSf_dBeqVm has a size of [nl, nBeqx]
        end
        for kk=1:nBeqz
            ...
            d2Sf_dBeqz2(kk,k) = (diag(Cf*V)*conj(d2Yf_dBeq2*V)).'*mu;    %AAB- must be zero           
        end
    end
end
