function [H14, H24, H34, H41, H42, H43, H44] = d2St_dxBeqv2(branch, V, mu, vcart)
%D2ST_DXBEQV2  Computes 2nd derivatives of complex brch power flow "to" w.r.t. BeqvVa, BeqvVm, BeqvBeqz, VaBeqv, VmBeqv, BeqzBeqv, BeqvBeqv.
%
...
%
%   [HBvVa, HBvVm, HBvBz, HVaBv, HVmBv, HBzBv, HBvBv] = D2ST_DXBEQV2(BRANCH, V, MU)
%   [HBvVa, HBvVm, HBvBz, HVaBv, HVmBv, HBzBv, HBvBv] = D2ST_DXBEQV2(BRANCH, V, MU, 0)
%
...
%
%   [HBvVa, HBvVm, HBvBz, HVaBv, HVmBv, HBzBv, HBvBv] = D2ST_DXBEQV2(BRANCH, V, MU, 1)
%
%   Not Coded Yet
%
%   Examples:
%       il = find(branch(:, RATE_A) ~= 0 & branch(:, RATE_A) < 1e10);
%       [HBvVa, HBvVm, HBvBz, HVaBv, HVmBv, HBzBv, HBvBv] = d2St_dxBeqv2(branch(il,:), V, mu, vcart);
%
%       Here the output matrices correspond to:
%           HBvVa = d/dBeqv (dSt_dVa.'   * mu)
%           HBvVm = d/dBeqv (dSt_dVm.'   * mu)
%           HBvBz = d/dBeqv (dSt_dBeqz.' * mu)
%           HVaBv = d/dVa   (dSt_dBeqv.' * mu)
%           HVmBv = d/dVm   (dSt_dBeqv.' * mu)
%           HBzBv = d/dBeqz (dSt_dBeqv.' * mu)
%           HBvBv = d/dBeqv (dSt_dBeqv.' * mu)
%
...
if vcart
    ...
    for k=1:nBeqv 
        for kk=1:nb %dBeqvVx
            ...
            d2St_dBeqvVa(kk, k) = ((diag(Ct*V)*conj(dYt_dBeq*dVa(:,kk))   +   (Ct*dVa(:,kk)).*conj(dYt_dBeq*V)).')*mu; %AAB- Final dSt_dBeqVa has a size of [nl, nBeqx] 
            d2St_dBeqvVm(kk, k) = ((diag(Ct*V)*conj(dYt_dBeq*dVm(:,kk))   +   (Ct*dVm(:,kk)).*conj(dYt_dBeq*V)).')*mu; %AAB- Final dSt_dBeqVm has a size of [nl, nBeqx] 
        end
        for kk=1:nBeqz
            %% Second Derivatives %The BeqvBeqz derivative is zero because a VSC type 1 cannot be a type 2 at the same time.
            d2Ytf_dBeqvBeqz = zeros(nl,1);                                    %AAB- must be zero            
            d2Ytt_dBeqvBeqz = zeros(nl,1);                                    %AAB- must be zero
            d2Yt_dBeqvBeqz = d2Ytf_dBeqvBeqz.*Cf + d2Ytt_dBeqvBeqz.*Ct;       %AAB- must be zero
            d2St_dBeqvBeqz(kk,k) = (diag(Ct*V)*conj(d2Yt_dBeqvBeqz*V)).'*mu;  %AAB- must be zero  
        end
        ...
        for kk=1:nBeqv
            ...
            d2St_dBeqv2(kk,k) = (diag(Ct*V)*conj(d2Yt_dBeq2*V)).'*mu;     %AAB- must be zero           
        end
    end
end
