function [H17, H27, H37, H47, H57, H67, H71, H72, H73, H74, H75, H76, H77] = d2Sf_dxvtma2(branch, V, mu, vcart)
%D2SF_DXVTMA2  Computes 2nd derivatives of complex brch power flow "from" w.r.t. vtmaVa, vtmaVm, vtmaBeqz, vtmaBeqv, vtmaSh, vtmaqtma, Vavtma, Vmvtma, Beqzvtma, Beqvvtma, Shvtma, qtmavtma, vtmavtma.
%
...
%   
%   [HvtmaVa, HvtmaVm, HvtmaBz, HvtmaBv, HvtmaSh, Hvtmaqtma, HVavtma, HVmvtma, HBzvtma, HBvvtma, HShvtma, Hqtmavtma, Hvtmavtma] = D2SF_DXVTMA2(BRANCH, V, MU)
%   [HvtmaVa, HvtmaVm, HvtmaBz, HvtmaBv, HvtmaSh, Hvtmaqtma, HVavtma, HVmvtma, HBzvtma, HBvvtma, HShvtma, Hqtmavtma, Hvtmavtma] = D2SF_DXVTMA2(BRANCH, V, MU, 0)
%
...
%
%   [HvtmaVa, HvtmaVm, HvtmaBz, HvtmaBv, HvtmaSh, Hvtmaqtma, HVavtma, HVmvtma, HBzvtma, HBvvtma, HShvtma, Hqtmavtma, Hvtmavtma] = D2SF_DXVTMA2(BRANCH, V, MU, 1)
%
%   Not Coded Yet
%
%   Examples:
...
if vcart
    ...
    %dYtf_dma = sparse( zeros(nl,nVtma) ); 
    %dYtt_dma = sparse( zeros(nl,nVtma) );
    ...
    d2Sf_dvtmash   = sparse( zeros(nPfsh,nVtma) );   
    ...
    for k=1:nVtma 
        for kk=1:nb %dQtmaVx
            ...
            %dYt_dvtma = dYtf_dvtma(:, k).* Cf + dYtt_dma(:, k).* Ct; %AAB- size [nl,nb] per active ma

            %2nd Derivatives of Sf w.r.t. VtmaVx
            d2Sf_dvtmaVa(kk, k) = ((diag(Cf*V)*conj(dYf_dvtma*dVa(:,kk))   +   (Cf*dVa(:,kk)).*conj(dYf_dvtma*V)).')*mu; %AAB- Final dSf_dmaVa has a size of [nl, nVtma] 
            d2Sf_dvtmaVm(kk, k) = ((diag(Cf*V)*conj(dYf_dvtma*dVm(:,kk))   +   (Cf*dVm(:,kk)).*conj(dYf_dvtma*V)).')*mu; %AAB- Final dSf_dmaVm has a size of [nl, nVtma] 
        end
        for kk=1:nBeqz
            ...
            d2Sf_dvtmaBeqz(kk,k) = (diag(Cf*V)*conj(d2Yf_dvtmaBeqz*V)).'*mu; %AAB- must be zero
        end
        for kk=1:nBeqv
            ...
            d2Sf_dvtmaBeqv(kk,k) = (diag(Cf*V)*conj(d2Yf_dvtmaBeqv*V)).'*mu; %AAB- must be zero
        end
        for kk=1:nPfsh
            ...
            d2Yft_dvtmash = ( 1j.*YsVtmashSel)./( k2.*abs(tap).*conj(tap) );  %AAB- Only active for elements with both Pf and Vt control
            %d2Ytf_dvtmash = (-1j.*YsVtmashSel)./( k2.*abs(tap).*     tap  ); %AAB- Only active for elements with both Pf and Vt control
            ...
            d2Sf_dvtmash(kk,k) = (diag(Cf*V)*conj(d2Yf_dvtmash*V)).'*mu;                  
        end
        for kk=1:nQtma
            %% Second Derivatives 
            ...
            d2Sf_dvtmaqtma(kk,k) = (diag(Cf*V)*conj(d2Yf_dvtmaqtma*V)).'*mu;                  
        end
        for kk=1:nVtma
            ...
            %d2Ytf_dma2 = sparse( ( -2.*YsVtma.*maSel2 )./( k2.*((abs(tap)).^2).*     tap  ) ); %AAB- Only when ma selector 1 and ma selector 2 match there will be a derivative (When k = kk). Otherwise zero
            %d2Ytt_dma2 = sparse( zeros(nl,1) );
            
            d2Yf_dma2 = d2Yff_dma2.*Cf + d2Yft_dma2.*Ct;          
            %d2Yt_dma2 = d2Ytf_dma2.*Cf + d2Ytt_dma2.*Ct;          
                  
            d2Sf_dvtma2(kk,k) = (diag(Cf*V)*conj(d2Yf_dma2*V)).'*mu;         %AAB- size of [nVtma, nVtma]   
...
H17 = sparse(d2Sf_dvtmaVa);
H27 = sparse(d2Sf_dvtmaVm);
H37 = sparse(d2Sf_dvtmaBeqz);
H47 = sparse(d2Sf_dvtmaBeqv);
H57 = sparse(d2Sf_dvtmash);
H67 = sparse(d2Sf_dvtmaqtma);
H71 = H17.';
H72 = H27.';
H73 = H37.';
H74 = H47.';
